...
@testset "Map fit interface" begin
    ## Set up series data
    tempdata = GeoArray.([rand(0.0:40.0, 10, 10) for i in 1:5], Ref((Lon, Lat)); name="stress", missingval=-99.0)
    ...
    modelkwargs = (period=2hr, nperiods=3, startdate=1hr)

    interface = mapfit!(wrapper, series, modelkwargs);
end
